...
import { decideOrgVerification } from "@/repositories/admin";
...
/** THE ANSWER (8 Sep 2026). An admin approves or rejects an organization —
 *  or revokes a verification given earlier. The RPC re-checks
 *  `is_platform_admin()` itself, so this action decides nothing; it validates
 *  the shape and carries the note. Approving lists every studio the
 *  organization owns; rejecting or revoking unlists them — that is the
 *  database's doing, in the same transaction as the tick. */
...
export async function decideOrgVerificationAction(input: unknown): Promise<{ error: string | null }> {
  ...
  try {
    await decideOrgVerification(supabase, {
