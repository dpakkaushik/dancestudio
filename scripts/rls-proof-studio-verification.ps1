...
try {
  ...
  $ta = Rpc (Api $a.token) "create_business_with_owner" @{ p_name = "SV Studio $stamp"; p_type = "studio"; p_area = "Kothrud"; p_city = "Pune" }
  ...
  # the businesses and the event are removed by hand because they hang off memberships
  try {
    if ($ta -and $ta.id) {
