...
$ta = Rpc (Api $owner.token) "create_business_with_owner" @{ p_name = "Pay Proof Studio $stamp"; p_type = "studio"; p_area = "Kothrud"; p_city = "Pune"; p_styles = @("Hip-Hop") }
Subscribe-Studio ([string]$ta.id)
$tb = Rpc (Api $rival.token) "create_business_with_owner" @{ p_name = "Rival Studio $stamp"; p_type = "studio"; p_area = "Andheri"; p_city = "Mumbai"; p_styles = @("Hip-Hop") }
...
try {
  ...
  $s2res = Invoke-RestMethod -Method Post -Uri "$base/rest/v1/class_sessions" -Headers $svcH -Body (@{
    class_id = $c1.id; business_id = $ta.id; starts_at = (& $mk $past2 19); ends_at = (& $mk $past2 20)
  ...
  $claim = Rpc (Api $owner.token) "ask_class_person" @{ p_class_id = $c1.id; p_user_id = $artist.id;
    p_kind = "artist"; p_can_attendance = $true; p_can_refunds = $false; p_pay_per_session_inr = 900 }
  Rpc (Api $artist.token) "respond_to_class_ask" @{ p_class_person_id = $claim.id; p_accept = $true } | Out-Null
  ...
  $trainerRateBlocked = Expect-Fail {
    Rpc (Api $trainer.token) "ask_class_person" @{ p_class_id = $c1.id; p_user_id = $ghost.id;
  ...
  $trainerSetPayBlocked = Expect-Fail {
    Rpc (Api $trainer.token) "set_class_person_pay" @{ p_class_person_id = $claim.id; p_pay_per_session_inr = 5000 }
  }
  ...
  $anonInsertBlocked = Expect-Fail {
    Invoke-RestMethod -Method Post -Uri "$base/rest/v1/payouts" -Headers $anonH -Body (@{
      business_id = $ta.id; user_id = $artist.id; amount_inr = 99999 } | ConvertTo-Json)
  }
  ...
  $trainerPayBlocked = Expect-Fail {
    RpcRaw (Api $trainer.token) "record_payout" "{""p_business_id"":""$($ta.id)"",""p_user_id"":""$($artist.id)"",""p_session_ids"":$(IdsJson @($s1))}"
  }
  ...
  $futureBlocked = Expect-Fail {
    RpcRaw (Api $owner.token) "record_payout" "{""p_business_id"":""$($ta.id)"",""p_user_id"":""$($artist.id)"",""p_session_ids"":$(IdsJson @($s3))}"
  }
  ...
  $payout = RpcRaw (Api $owner.token) "record_payout" "{""p_business_id"":""$($ta.id)"",""p_user_id"":""$($artist.id)"",""p_session_ids"":$(IdsJson @($s1,$s2)),""p_method"":""upi"",""p_status"":""done"",""p_provider_ref"":""UTR-$stamp""}"
  ...
  $doublePayBlocked = Expect-Fail {
    RpcRaw (Api $owner.token) "record_payout" "{""p_business_id"":""$($ta.id)"",""p_user_id"":""$($artist.id)"",""p_session_ids"":$(IdsJson @($s1))}"
  }
  ...
  Rpc (Api $owner.token) "set_class_person_pay" @{ p_class_person_id = $claim.id; p_pay_per_session_inr = 1500 } | Out-Null
  ...
  $repay = RpcRaw (Api $owner.token) "record_payout" "{""p_business_id"":""$($ta.id)"",""p_user_id"":""$($artist.id)"",""p_session_ids"":$(IdsJson @($s1))}"
  ...
  $gclaim = Rpc (Api $owner.token) "ask_class_person" @{ p_class_id = $c1.id; p_user_id = $ghost.id;
    p_kind = "assistant"; p_can_attendance = $true; p_can_refunds = $false; p_pay_per_session_inr = 500 }
  Rpc (Api $ghost.token) "respond_to_class_ask" @{ p_class_person_id = $gclaim.id; p_accept = $true } | Out-Null
  Rpc (Api $owner.token) "remove_business_member" @{ p_business_id = $ta.id; p_user_id = $ghost.id } | Out-Null
  $ghostClaimGone = Get-Rows $svcH "class_people?id=eq.$($gclaim.id)&deleted_at=is.null&select=id"
  $ghostPay = RpcRaw (Api $owner.token) "record_payout" "{""p_business_id"":""$($ta.id)"",""p_user_id"":""$($ghost.id)"",""p_session_ids"":$(IdsJson @($s2))}"
  ...
  $zclaim = Rpc (Api $owner.token) "ask_class_person" @{ p_class_id = $c1.id; p_user_id = $trainer.id;
    p_kind = "assistant"; p_can_attendance = $false; p_can_refunds = $false; p_pay_per_session_inr = 0 }
  Rpc (Api $trainer.token) "respond_to_class_ask" @{ p_class_person_id = $zclaim.id; p_accept = $true } | Out-Null
  $zeroBlocked = Expect-Fail {
    RpcRaw (Api $owner.token) "record_payout" "{""p_business_id"":""$($ta.id)"",""p_user_id"":""$($trainer.id)"",""p_session_ids"":$(IdsJson @($s1))}"
  }
  ...
  $crossBlocked = Expect-Fail {
    RpcRaw (Api $owner.token) "record_payout" "{""p_business_id"":""$($ta.id)"",""p_user_id"":""$($artist.id)"",""p_session_ids"":$(IdsJson @($s4))}"
  }
