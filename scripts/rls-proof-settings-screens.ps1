...
try {
  ...
  $cat = Rows (Api $dancer.token) "plans?select=key,price_inr,active&key=eq.artist_monthly"
...
function Subscribe-Studio($tenantId) {
  ...
  $t = Rpc (Api $owner.token) "create_business_with_owner" @{ p_name = "Settings Proof Studio $stamp"; p_type = "studio"; p_area = "Kothrud"; p_city = "Pune" }
  ...
  $r12direct = Fails { Invoke-RestMethod -Method Patch -Uri "$base/rest/v1/businesses?id=eq.$tenantId" -Headers (Plain $owner.token) -Body (@{ type = "artist_page" } | ConvertTo-Json) }
  Invoke-RestMethod -Method Patch -Uri "$base/rest/v1/businesses?id=eq.$tenantId" -Headers $svcH -Body (@{ verified_at = "2026-08-29T00:00:00Z" } | ConvertTo-Json) | Out-Null
  $row12 = Rows $anonH "businesses?select=area,type,verified_at&id=eq.$tenantId"
  ...
  if ($tenantId) { try { Invoke-RestMethod -Method Delete -Uri "$base/rest/v1/businesses?id=eq.$tenantId" -Headers $svcH | Out-Null } catch {} }
