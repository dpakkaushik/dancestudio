...
function Count-Of($headers, $tenantId) {
  $rows = @((Rpc $headers "follower_counts" @{ p_business_ids = @($tenantId) }) | Where-Object { $null -ne $_ })
...
$ta = New-Studio $ownerA.token "Follow Proof Studio $stamp" "Kothrud" "Pune"
Subscribe-Studio ([string]$ta.id)
$tb = New-Studio $ownerB.token "Private Studio $stamp" "Andheri" "Mumbai"
...
try {
  ...
  $anonA = Get-Rows $anonH "businesses?id=eq.$($ta.id)&select=id,name,type,created_at"
  $anonB = Get-Rows $anonH "businesses?id=eq.$($tb.id)&select=id"
  ...
  $f1 = Rpc (Api $l1.token) "set_follow" @{ p_business_id = $ta.id; p_on = $true }
  $f1b = Rpc (Api $l1.token) "set_follow" @{ p_business_id = $ta.id; p_on = $true }
  ...
  $direct = Fails { Invoke-RestMethod -Method Post -Uri "$base/rest/v1/follows" -Headers (Api $l2.token) -Body (@{ follower_id = $l2.id; business_id = $ta.id } | ConvertTo-Json) }
  Check 4 "A direct insert into follows is refused ($direct)" ($direct -ne "")

  # 5. a second follower; the public count moves
  $f2 = Rpc (Api $l2.token) "set_follow" @{ p_business_id = $ta.id; p_on = $true }
  ...
  $l1Rows = Get-Rows (Api $l1.token) "follows?business_id=eq.$($ta.id)&deleted_at=is.null&select=id,follower_id"
  $ownerRows = Get-Rows (Api $ownerA.token) "follows?business_id=eq.$($ta.id)&deleted_at=is.null&select=id,follower_id,profiles!follows_follower_id_fkey(full_name)"
  $rivalRows = Get-Rows (Api $ownerB.token) "follows?business_id=eq.$($ta.id)&select=id"
  $anonRows = Get-Rows $anonH "follows?business_id=eq.$($ta.id)&select=id"
  ...
  $u1 = Rpc (Api $l1.token) "set_follow" @{ p_business_id = $ta.id; p_on = $false }
  $ended = Get-Rows (Api $l1.token) "follows?follower_id=eq.$($l1.id)&business_id=eq.$($ta.id)&select=id,deleted_at"
  $r1 = Rpc (Api $l1.token) "set_follow" @{ p_business_id = $ta.id; p_on = $true }
  $all1 = Get-Rows (Api $l1.token) "follows?follower_id=eq.$($l1.id)&business_id=eq.$($ta.id)&select=id,deleted_at"
  ...
  $pubSched = Get-Rows $anonH "class_sessions?select=id,classes!inner(status,title)&business_id=eq.$($ta.id)&classes.status=eq.published&deleted_at=is.null&starts_at=gte.$nowQ"
  $pubAll = Get-Rows $anonH "class_sessions?select=id,classes!inner(status)&business_id=eq.$($ta.id)&deleted_at=is.null"
  $ownerAll = Get-Rows (Api $ownerA.token) "class_sessions?select=id,classes!inner(status)&business_id=eq.$($ta.id)&deleted_at=is.null"
  ...
  $styles = Get-Rows $anonH "classes?select=style&business_id=eq.$($ta.id)&status=eq.published&deleted_at=is.null"
  $allStyles = Get-Rows $anonH "classes?select=style,status&business_id=eq.$($ta.id)&deleted_at=is.null"
